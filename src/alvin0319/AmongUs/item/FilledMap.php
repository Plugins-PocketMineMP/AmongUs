...
namespace alvin0319\AmongUs\item;
...
class FilledMap extends Item{
	...
	public function updateData() : void{
		if($this->getNamedTag()->hasTag("mapData", ByteArrayTag::class)){
			...
			foreach($mapData as $y => $_x){
				foreach($_x as $x => $color){
			...
			$packet->colors = $res;
			$packet->height = $packet->width = 128;
			$packet->scale = 1;
	...
	public function getMapId() : int{
		return $this->getNamedTag()->getLong("map_uuid", 0);
	}
	...
	public function getClientboundMapItemDataPacket() : ?ClientboundMapItemDataPacket{
		return $this->mapPacket === null ? null : clone $this->mapPacket;
	}
}